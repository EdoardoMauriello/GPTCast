...
class Encoder(nn.Module):
    def __init__(self, *, in_ch, out_ch, z_ch, hid_ch, hid_ch_mult=(1,2,4,8),
        ...
        # curr_res = resolution
        in_ch_mult = (1,)+tuple(hid_ch_mult)
        self.in_ch_mult = in_ch_mult
        self.down = nn.ModuleList()
        for i_level in range(self.num_resolutions):
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_in = hid_ch*in_ch_mult[i_level]
...
class Decoder(nn.Module):
    def __init__(self, *, in_ch, out_ch, z_ch, hid_ch, hid_ch_mult=(1,2,4,8),
        ...
        block_in = hid_ch*hid_ch_mult[self.num_resolutions-1]
        ...
        # z to block_in
        self.conv_in = torch.nn.Conv2d(z_ch,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
